...
select booking_id,user_id,match_id,coalesce(payment_status,'Action Required') as payment_status from bookings where payment_status isnull 

-- Query 4: Retrieve match booking details along with the User's full name and the scheduled Match fixture teams.
select b.booking_id,u.full_name,m.fixture,b.total_cost from users u inner join bookings b on u.user_id=b.user_id inner join matches m on m.match_id=b.match_id
...
select match_id,fixture,base_ticket_price from matches order by base_ticket_price desc offset 1 row fetch next 2 rows only
